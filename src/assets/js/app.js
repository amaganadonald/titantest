...
export const appjs = () => {
  ...
  function h() {
    //console.log(JSON.parse(sessionStorage.getItem('defaultAttribute')))
    var o,
    ...
    !t ||
      ...
      ((o = document.createElement('ul')).innerHTML =
        '<a href="#" class="logo"><img src="../images/logo.png" alt="" height="22"></a>'),
      Array.from(
    ...
    Array.from(o).forEach(function (e, t) {
      t + 1 === a && (s = e),
        ...
        t + 1 === o.length &&
          ...
          s.insertAdjacentHTML(
            'afterend',
            '<li class="nav-item">\t\t\t\t\t\t<a class="nav-link" href="#sidebarMore" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="sidebarMore">\t\t\t\t\t\t\t<i class="ri-briefcase-2-line"></i> More\t\t\t\t\t\t</a>\t\t\t\t\t\t<div class="collapsemenu-dropdown" id="sidebarMore"><ul class="nav nav-sm flex - column">' +
              n +
              '</ul></div>\t\t\t\t\t</li>'
          );
    });
  ...
  function A() {
    ...
    var e,
      t,
      a = '/' == location.pathname ? 'index' : location.pathname.substring(1);
      ...
      0 < e.querySelectorAll('.notification-item').length
        ...
        : (e.querySelector('.view-all') &&
          ...
          e.querySelector('.empty-notification-elem') ||
            (e.innerHTML +=
              '<div class="empty-notification-elem">\t\t\t\t\t\t\t<div class="w-25 w-sm-50 pt-3 mx-auto">\t\t\t\t\t\t\t\t<img src="../images/svg/bell.svg" class="img-fluid" alt="user-pic">\t\t\t\t\t\t\t</div>\t\t\t\t\t\t\t<div class="text-center pb-5 mt-2">\t\t\t\t\t\t\t\t<h6 class="fs-18 fw-semibold lh-base">Hey! You have no any notifications </h6>\t\t\t\t\t\t\t</div>\t\t\t\t\t\t</div>'));
    });
  }
  ...
  sessionStorage.getItem('defaultAttribute')
    ...
    document.getElementsByClassName('nc') &&
      (H(),
      Array.from(document.querySelectorAll('.nc input')).forEach(function (t) {
        t.addEventListener('change', function (e) {
          e.target.closest('.nc').classList.toggle('active');
          var t = document.querySelectorAll('.nc input:checked').length;
          e.target.closest('.nc-item').classList.contains('active'),
            (document.getElementById('nc').style.display =
              0 < t ? 'block' : 'none'),
