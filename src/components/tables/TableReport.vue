          ...
          <div class="col-sm-4">
            <div class="search-box" v-if="$props.tb === 'transport'">
              <input
                ...
                class="form-control"
                id="search-opts"
                placeholder="Parcèlles séparée par un espace"
                v-model="piste"
                @blur="(event) => changeParcel()"
              />
          ...
          <div class="col-sm-auto ms-auto">
            <div class="list-grid-nav hstack gap-1">
              ...
              <button
                ...
                @click="generer()"
                v-if="$props.tb === 'transport'"
              >
                <q-tooltip>générer rapport</q-tooltip> Générer
              </button>
              <button
                type="button"
                class="btn btn-primary"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const calculReport = () => {
      ...
      if (props.tb === 'horaire') {
        store2.allActivites(
          moment(datedeb.value).format('YYYY-MM-DD HH:mm:ss'),
          moment(datefin.value).format('YYYY-MM-DD HH:mm:ss')
        );
      } else if (props.tb === 'distance') {
        store2.allActivitesKm(
          moment(datedeb.value).format('YYYY-MM-DD HH:mm:ss'),
          moment(datefin.value).format('YYYY-MM-DD HH:mm:ss')
        );
